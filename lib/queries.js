...
module.exports = function createQueryBuilder({ model, strapi }) {
  ...
  function find(params, populate, { transacting } = {}) {
    ...
    const sortableManyRelatedCollections = model.associations
      ?.filter(assoc => ["belongsToMany", "manyWay"].includes(assoc.nature) && assoc.tableCollectionName && assoc.alias)
      ?.map(assoc => ({ [assoc.alias]: (qb) => { qb.orderBy('_pivot_id', 'asc'); } }))

    const withRelated = model.uid.startsWith("application::") ? [
      ...(populate ?? []),
      ...(sortableManyRelatedCollections ?? []),
    ] : populate;
